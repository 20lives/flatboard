...
function createWallGeometry(
  ...
  const outerWallGeometry = cube([
    ...
    wallHeight,
  ]).translate([plateWidth / 2, plateHeight / 2, wallCenterZPosition]);

  const innerCavityGeometry = cube([plateWidth, plateHeight, wallHeight + extrusionTolerance]).translate([
    plateWidth / 2,
    plateHeight / 2,
    wallCenterZPosition,
  ]);
