...
const createRotatedSquareGeometry = (
  ...
  square([keySize, keySize], { center: true })
    .rotate_z(rotationDegrees)
    .linear_extrude(extrusionHeight)
    .translate([centerXPosition, centerYPosition, 0]);

export const createAllSwitchCutouts = (keyPlacements: KeyPlacement[], size: number, height: number) =>
  keyPlacements.map(({ pos: { x, y }, rot }) => createRotatedSquareGeometry(size, height, x, y, rot));
