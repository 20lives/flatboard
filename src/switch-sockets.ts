 ...
 */
function createRotatedSquareGeometry(
  keySize: number,
 ...
 */
export function createKeyCutoutGeometry(
  ...
  return keyPlacements.map(({ pos, rot }) =>
    createRotatedSquareGeometry(cutoutSize, pos.x + plateOffset.x, pos.y + plateOffset.y, rot, cutoutHeight).translate([
      0,
      0,
      zPositionOffset,
    ]),
 ...
 */
export function createSwitchHoleCutouts(
  keyPlacements: KeyPlacement[],
  plateOffset: Point2D,
  generalClearance: number,
  effectiveTotalThickness: number,
  effectiveCutoutSize: number,
) {
 ...
 */
export function createThinZoneCutouts(
  keyPlacements: KeyPlacement[],
  plateOffset: Point2D,
  frameStructureHeight: number,
  generalClearance: number,
  effectiveThinZone: number,
) {
  return createKeyCutoutGeometry(
    keyPlacements,
    plateOffset,
 ...
 */
export function createFrameOuterBoundCutouts(
  keyPlacements: KeyPlacement[],
  plateOffset: Point2D,
  frameWallThickness: number,
...
export function createAllSwitchCutouts(keyPlacements: KeyPlacement[], plateOffset: Point2D, config: KeyboardConfig) {
  ...
  const switchHoleCutouts = createSwitchHoleCutouts(
    keyPlacements,
    plateOffset,
    config.tolerances.general,
    config.switch.plate.totalThickness,
    config.switch.cutout.size,
  );
  const thinZoneCutouts = createThinZoneCutouts(
    keyPlacements,
    plateOffset,
    frameStructureHeight,
    config.tolerances.general,
